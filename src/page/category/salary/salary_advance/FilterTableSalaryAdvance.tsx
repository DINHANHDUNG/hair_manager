...
export default function FilterTableAdvanced({ open, anchorRef, handleClose, handleComfirm, value }: Props) {
  ...
  return (
    <PopperComponent clickAway open={open} anchorRef={anchorRef} handleClose={handleClose}>
      ...
      <Box sx={{ p: 2, pb: 0 }}>
        <InputLabel sx={{ mb: 1 }} htmlFor={`input-Autocomplete`}>{`Hoàn ứng`}</InputLabel>
        <Autocomplete
          ...
          value={
            state.isRefund
              ? {
                  ...
                  label: OPTION_COMPLETION.find((opt) => opt.value === state.isRefund)?.label || ''
                }
              : null
          }
